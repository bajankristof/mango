...
attempt(LastAttemptNo, {Module, Function, Args}, Opts) ->
    ...
    case {erlang:apply(Module, Function, Args), LastAttemptNo + 1} of
        ...
        {{error, Reason}, MaxAttempts} ->
            logger:notice("mango: ~ts:~ts failed due to ~p", [Module, Function, Reason]),
            logger:warning("mango: ~ts:~ts exhausted retry attempts, shutting down", [Module, Function]),
            {error, Reason};
